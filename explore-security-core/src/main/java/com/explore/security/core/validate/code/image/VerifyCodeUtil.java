...
public class VerifyCodeUtil {
    ...
    public static BufferedImage generateImageCode(String textCode) {
        return generateImageCode(textCode, 90, 30, 7, true, null, null, null);
    }
    /**
     * 生成图片验证码
     * @param width          图片宽度(注意此宽度若过小,容易造成验证码文本显示不全,如4个字符的文本可使用85到90的宽度)
     ...
     * @return 图片缓存对象
     */
    public static BufferedImage generateImageCode(String textCode, Color backColor, Color foreColor, Color lineColor) {
        return generateImageCode(textCode, 90, 30, 7, true, backColor, foreColor, lineColor);
    }
